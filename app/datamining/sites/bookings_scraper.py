...
def bookings_from_pages(site: str, pages) -> dict[str, str]:
...
async def collect_bookings(site: str, max_pages: int = 40) -> dict[str, str]:
    ...
    return bookings_from_pages(site, pages)
...
def write_results_csv(path: str | Path, rows: list[dict[str, str]]) -> None:
    ...
    with output_path.open("w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=BOOKING_FIELDNAMES)
        writer.writeheader()
        for row in rows:
            writer.writerow({name: row.get(name, "") for name in BOOKING_FIELDNAMES})


def cli() -> None:
    parser = argparse.ArgumentParser(description="Detect booking forms and CRM widgets on a site.")
    parser.add_argument("--site", required=True)
    parser.add_argument("--max-pages", type=int, default=40)
    parser.add_argument("--output-csv", required=True)
    args = parser.parse_args()

    row = asyncio.run(collect_bookings(args.site, max_pages=max(1, args.max_pages)))
    write_results_csv(args.output_csv, [row])
    print(f"Saved 1 row to {args.output_csv}")
